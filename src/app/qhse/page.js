  ...
  return (
    <div className="min-h-screen bg-transparent text-white px-6 py-10">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Quality, Health, Safety & Environment</h1>
          <p className="text-sm text-slate-200 mt-2">
            Right now we don&apos;t have any QHSE data. It will arrive in the future.
          </p>
        </div>

        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur shadow-xl space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            {["Open NCRs", "Incidents", "Audits"].map((label, idx) => (
              <div
                key={label}
          ...
          <p className="text-sm text-slate-200">
            Placeholder cards for QHSE metrics. Data will be shown here once connected.
          </p>
        </div>
      </div>
    </div>
