...
export default function EquipmentListContent() {
  ...
  const statusClass = (status) => {
    if (status === "AVAILABLE") {
      return "bg-emerald-500/15 border border-emerald-400/40 text-emerald-100";
    }
    if (status === "IN_USE") {
      return "bg-amber-500/15 border border-amber-400/40 text-amber-100";
    }
    return "bg-slate-500/15 border border-slate-400/40 text-slate-100";
  };
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/pms/equipment-used/list");
      const data = await res.json();
      setItems(data?.equipmentUsed || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!retirementDate) {
      setError("Retirement date is required");
      return;
    }
    try {
      setLoading(true);
      const res = await fetch("/api/pms/equipment-used/create", {
        method: "POST",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(`/api/pms/equipment-used/${id}/delete`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  return (
    <div className="space-y-6">
      {/* Add Equipment Form */}
      <div className="rounded-2xl border border-white/10 bg-linear-to-br from-white/5 to-white/2 p-6 backdrop-blur-xl shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-white/60 mt-0.5">
              Register new equipment to track usage
            </p>
          </div>
        </div>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-white/50">
                Availability is auto-managed by STS operations.
              </p>
            </div>
          </div>
      ...
      {/* Equipment Table */}
      <div className="rounded-2xl border border-white/10 bg-linear-to-br from-white/5 to-white/2 backdrop-blur-xl shadow-2xl overflow-hidden">
              ...
              {items.map((item, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="font-semibold text-white">{item.equipmentName}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-semibold ${statusClass(
                    ...
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40 mt-1">
                          Add your first equipment above to get started
                        </p>
                      </div>
                    </div>
                  </td>
